"""Create notebooks/05_mult_emotion_training.ipynb notebook."""
import json
  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "    get_ipython().system(f'gsutil cp gs://{GCS_BUCKET}/checkpoints/phase1/{BEST_CHECKPOINT_NAME} {config.paths.checkpoints_dir}/{BEST_CHECKPOINT_NAME} || true')\n",
        "    get_ipython().system(f'gsutil cp gs://{GCS_BUCKET}/checkpoints/phase1/{LAST_CHECKPOINT_NAME} {config.paths.checkpoints_dir}/{LAST_CHECKPOINT_NAME} || true')\n",
        "    get_ipython().system(f'mkdir -p {config.paths.logs_dir}')\n",
        "    get_ipython().system(f'gsutil cp gs://{GCS_BUCKET}/logs/phase1/history.csv {config.paths.logs_dir}/history.csv || true')\n",
        "\n",
        "if WANDB_ENABLE:\n",
        "    import wandb\n",
        "    wandb.login()\n",
...
dest_file = Path("d:/Hai/study/DeepLerning/BCDA/notebooks/05_mult_emotion_training.ipynb")
dest_file.parent.mkdir(parents=True, exist_ok=True)
with open(dest_file, "w", encoding="utf-8") as f:
    json.dump(notebook_content, f, indent=2, ensure_ascii=False)
print("Successfully generated notebooks/05_mult_emotion_training.ipynb")
